...
def safe_patch(filepath):
    ...
    # 2. Update gold variables and gold button styles to dark slate/navy
    ...
    # Replace inline gold colors
    gold_colors = ['#9e793e', '#d4af37', '#a68a56', '#c9a050', '#FFC72C', '#8b7348', '#86642d']
    for gc in gold_colors:
        html = html.replace(gc, '#0f172a')

    # 3. Hide / Remove Acolhedor Bar
    html = html.replace('.acolhedor-bar {', '.acolhedor-bar {\n            display: none !important;')

    # 4. Insert #mode-child HTML right before #mode-visitor
    mode_child_block = '''            <!-- MODE CHILD (CRIANÇAS CHECK-IN) -->
...
'''
    ...
    if 'id="mode-child"' not in html:
        target_marker = '<!-- 1. VISITOR CONTENT -->'
        if target_marker in html:
            html = html.replace(target_marker, mode_child_block + target_marker)
        else:
            # Fallback insert before id="mode-visitor"
            html = html.replace('<div id="mode-visitor"', mode_child_block + '            <div id="mode-visitor"')

    # Update switchCheckinMode to call loadAllKidsList when mode === 'child'
    old_target_code = 'if (target) {\n                    target.classList.add(\'active\');'
    new_target_code = 'if (target) {\n                    target.classList.add(\'active\');\n                    if (mode === \'child\' && typeof window.loadAllKidsList === \'function\') window.loadAllKidsList();'
    if old_target_code in html and 'loadAllKidsList' not in html[html.find('switchCheckinMode'):]:
        html = html.replace(old_target_code, new_target_code)

    # 5. Remove icons (<i class="..."></i> and loose </i>)
    ...
    print("Safe patch executed successfully.")

safe_patch('recepcao_v2.html')
